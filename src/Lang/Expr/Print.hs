...
prettyPrint = foldExpr phi
  where phi expr = case expr of
         ...
         Builtin f   -> show f
         Transpose x -> x ++ "^T"
         Row _ v     -> "(" ++ intercalate ", " v ++ ")"
         Col _ v     -> "(" ++ intercalate ", " v ++ ")^T"
...
printATree tree = foldAExpr printTree'
  where printTree' t expr = case expr of
         ...
         Case _ _    -> error "TODO"
         Builtin f   -> show f
         Transpose x -> tree t "transpose" [x]
         Row n v     -> tree t "row" (show n : v)
         Col n v     -> tree t "col" (show n : v)
